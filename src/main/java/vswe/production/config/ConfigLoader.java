    ...
    public static void init(File file) {
        ...
        for (Upgrade upgrade : Upgrade.values()) {
            ...
            if (max.getConfigurableMax() > 0) {
                upgrade.getMaxCountObject().setMax(config.getInt(upgrade.getUnlocalizedName() + MAX_COUNT_SUFFIX, UPGRADES, max.getMax(), 0, max.getConfigurableMax(), "Max count of the " + Localization.translate(upgrade.getUnlocalizedName()) + " upgrade"));
            }
        }
